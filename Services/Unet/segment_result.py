...
class UnetInterface(object):
    ...
    def __init__(self):
        self.prefix = '/home/kyfq/MyPython/PycharmProjects/qas/Services/Unet/segnet_bb5_final'
        self.epoch = 0
        self.seg_data_shape = 128
        self.batch_size = 1
        self.ctx = mx.gpu(0)

        self.sym, self.arg_params, self.aux_params = mx.model.load_checkpoint(self.prefix, self.epoch)
        self.mod = mx.mod.Module(symbol=self.sym, context=self.ctx, data_names=['data'], label_names=None)
        self.mod.bind(
            for_training=False,
            data_shapes=[('data', (self.batch_size, 3, self.seg_data_shape, self.seg_data_shape))],
            label_shapes=None
        )
    ...
    def segment(self, detect_dir, save_dir, flag='unet'):
        ...
        for file_name in yolo_detect_cells_list:
            ...
            # 图像亮度处理
            raw_img_light = self.contrast_brightness_image(raw_img)

            # 分割
            pred = self.seg_img(raw_img_light, self.mod).astype(np.uint8)

            # 找最大连同区域
            pred, contours, max_area, max_perimeter = self.find_max_contour(pred)
...
if __name__ == "__main__":
        ...
        p, c = unet_instance.segment('/home/kyfq/MyPython/PycharmProjects/qas/Services/Yolo/detected_img/',
